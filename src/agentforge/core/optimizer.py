...
Now receives a Config object instead of raw api_key/api_base/model.
"""
...
class SelfOptimizer:
    def __init__(self, agent_id: str, config=None):
        ...
        if config:
            self.cfg = config
            llm_cfg = config.get_llm_config(agent_id)
        ...
        # Lightweight LLM client for self-evaluation — uses cheapest model
        eval_model = os.environ.get("MODEL_EVAL", "qwen-turbo")
        self._eval_llm = LLMClient(
            api_key=self.api_key, api_base=self.api_base, model=eval_model,
            max_retries=2,
        )

    def reflect_on_task(self, task_desc: str, tool_output: str,
                        llm_response: str, time_taken: float) -> dict:
...
请直接回复数字（1-5），不要解释。"""
    ...
    def _generate_improvement(self, task: str, tool_out: str,
                               llm_resp: str, quality: int) -> Optional[str]:
        if quality >= 4:
            return None
...
只回复建议内容，不要其他内容。"""
        ...
        return resp[:200] if resp else None

    def _update_rules(self, improvement: str):
        rules = []
    ...
    def print_status(self):
        ...
        for t, p in self.memory.data["task_patterns"].items():
            if p["count"] > 2:
                print(f"\n{t}: 成功率 {p.get('success_rate', 0):.1%}, "
                      f"平均时间 {p.get('avg_time', 0):.1f}s")
